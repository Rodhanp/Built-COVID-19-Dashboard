...
with open('D:\Other Documents\Coursera\Building COVID-19 Dashboard Using Dash & Plotly\Script\cc3_cn_r.json') as json_file:
...
#1st jan 	  0		afghanistan
#2nd jan  	  0
df_excluded_recovered=df_time_recovered[df_time_confirmed.columns\
...
print(df_date_recovered.head())

countries=df_date_recovered['Country/Region'].unique()
## LOADING NEWS ARTICLES

news_articles_list = open("D:\Other Documents\Coursera\Building COVID-19 Dashboard Using Dash & Plotly\Script\covid_news_articles.csv", "r",encoding='utf-8').readlines()
...
#				style={'fontSize':12}),
		...
		,width=2)
	new_cards_list.append(news_cards_template)
...
card_active_cases=dbc.Card(
	...
	dbc.CardBody(
		...
		dbc.CardLink("See more Details",href="https://github.com/CSSEGISandData")])])
...
dropdown_vaccine_timeline = dcc.Dropdown(
	id='dropdown_vaccination_timeline',
...
app.layout = html.Div(children=[navbar,
    ...
    #INDICATOR
    dbc.Row([dbc.Col(dcc.Graph(figure=fig_indicator),width=2)],justify='center',align="center"),
    ...
    dbc.Row([dbc.Col(dcc.Graph(id='vaccine-timeline'),width=10)],justify='center'),
    ...
    html.Br(),
    #NEWS CARDS
    dbc.Row(new_cards_list, justify='center')
    ]
    )
...
@app.callback(
	...
	[Input('dropdown_vaccination_timeline','value')])
def vaccine_timeline(country):
	...
	fig.add_trace(go.Scatter(x=df_vacc_filtered['date'],\
		y=df_vacc_filtered['total_vaccinations'],fill='tonexty',name='Total Vaccinations'))
	fig.add_trace(go.Scatter(x=df_vacc_filtered['date'],\
		y=df_vacc_filtered['people_fully_vaccinated'],fill='tozeroy',name='People Fully Vaccinated'))
